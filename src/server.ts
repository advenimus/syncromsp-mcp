...
export function createServer(
  ...
  let allToolsCache: DomainTool[] | null = null;

  async function getAllTools(): Promise<DomainTool[]> {
    if (allToolsCache) return allToolsCache;

    const all: DomainTool[] = [];
  ...
  const toolHandlerMap = new Map<string, DomainTool>();

  async function ensureToolMap(): Promise<void> {
    if (toolHandlerMap.size > 0) return;
  ...
  if (toolMode === "flat") {
    ...
    server.setRequestHandler(
      ...
      async (request): Promise<CallToolResult> => {
        ...
        try {
          ...
          if (!tool) {
            return errorResult(
              `Unknown tool: ${name}. Use a tool from the available list.`
            );
          }
    ...
    const navigationTools: Tool[] = [
      ...
      },
      {
        name: "syncro_back",
        description:
          "Return to the root domain list from the current domain",
        inputSchema: { type: "object" as const, properties: {} },
      },
    ];
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      const backTool = navigationTools.find(
        (t) => t.name === "syncro_back"
      )!;
      ...
      async (request): Promise<CallToolResult> => {
        ...
        try {
          ...
          if (name === "syncro_back") {
            ...
            if (!session.currentDomain) {
              return textResult(
                "Already at root. Use syncro_navigate to enter a domain."
              );
            }
